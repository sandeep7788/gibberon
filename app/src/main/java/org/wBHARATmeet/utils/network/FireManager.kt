...
import org.wBHARATmeet.extensions.getFileRx
import org.wBHARATmeet.extensions.observeSingleValueEvent
import org.wBHARATmeet.extensions.setValueRx
import org.wBHARATmeet.extensions.snapshotAtRefExists
import org.wBHARATmeet.model.ImageItem
import org.wBHARATmeet.model.constants.LastSeenStates
...
class FireManager {
    ...
    fun removeMyPhoto()  {
            ...
            //save them in firebase database using one request
          RxFirebaseDatabase.updateChildren(FireConstants.usersRef.child(uid), updateMap)


    }


    fun removeGroupImage(groupId: String) {
        ...
        RxFirebaseDatabase.updateChildren(
            FireConstants.groupsRef.child(groupId).child("info"),
            updateMap
        )
...
//this will upload the user photo that he picked and generate a Small circle image and decode as base64
fun removeBrodcastImage(groupId: String) {
    ...
    updateMap["thumbImg"] = "null"
    RxFirebaseDatabase.updateChildren(
        FireConstants.broadcastsRef.child(groupId).child("info"),
        updateMap
    )
}
...
//update voice message state as Seen
    ...
    fun downloadBroadcastPhoto(uid: String, oldLocalPath: String?, isBroadcast: Boolean): Single<String> {
        if (imageDownloadProcessIds.contains(uid)) return Single.error(Throwable("already downloading"))

        imageDownloadProcessIds.add(uid)
        val ref =  FireConstants.broadcastsRef.child(uid).child("info")
    ...
    companion object {
        ...
        fun checkAndDownloadUserPhoto(user: User?): Observable<ImageItem> {
            ...
            var databaseReference =
                    when {
                        user.isGroupBool -> FireConstants.groupsRef.child(user.uid).child("info")
                        user.isBroadcastBool -> FireConstants.broadcastsRef.child(user.uid).child("info")
                        else -> FireConstants.usersRef.child(user.uid)
                    }
