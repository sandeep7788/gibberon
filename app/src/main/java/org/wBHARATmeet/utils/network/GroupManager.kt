...
import org.wBHARATmeet.model.constants.GroupEventTypes.GROUP_CREATION
import org.wBHARATmeet.model.constants.GroupEventTypes.USER_ADDED
...
//this will upload the user photo that he picked and generate a Small circle image and decode as base64
    ...
    fun removeMyPhoto()  {
        ...
        RxFirebaseDatabase.updateChildren(FireConstants.usersRef.child(FireManager.uid), updateMap)
...
fun changeGroupImage(imagePath: String, groupId: String): Completable {
    ...
    if (imagePath.equals("null")) {
        ...
        fileName = UUID.randomUUID().toString() + Util.getFileExtensionFromPath(imagePath)
        ref=FireConstants.imageProfileRef.child(fileName)
        file=Uri.fromFile(File(imagePath))
    }
...
//this will upload the user photo that he picked and generate a Small circle image and decode as base64
fun changeBrodcastImage(imagePath: String, groupId: String): Completable {
    ...
    return ref.putFileRx(file).flatMapMaybe {
        ...
        //save them in firebase database using one request
        return@flatMapCompletable RxFirebaseDatabase.updateChildren(
            FireConstants.broadcastsRef.child(
                groupId
            ).child("info"), updateMap
        )
    }
...
fun fetchUserGroups(): Observable<List<User>> {
    ...
    return RxFirebaseDatabase.observeSingleValueEvent(FireConstants.groupsByUser.child(FireManager.uid))
