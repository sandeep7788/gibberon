...
import org.wBHARATmeet.adapters.messaging.holders.*
import org.wBHARATmeet.adapters.messaging.holders.base.BaseHolder
import org.wBHARATmeet.adapters.messaging.holders.base.ReceivedDeletedMessageHolder
...
import org.wBHARATmeet.DisappearHolder
...
class MessagingAdapter(private val mOnClickListener: OnClickListener, private val messages: OrderedRealmCollection<Message>, autoUpdate: Boolean,
    ...
    override fun getItemViewType(position: Int): Int {
        ...
        if (user.isBlocked && position == messages.size - 1) {
            type= MessageType.USER_BLOCKED_MSG
        }
        return type
    }
    ...
    override fun onBindViewHolder(mHolder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (type) {
            ...
            MessageType.DISAPPEAR_MAG -> {
                val sentTextHolder = mHolder as DisappearHolder
                initHolder(sentTextHolder)
                sentTextHolder.bind(message, user)
            }
            ...
            MessageType.USER_BLOCKED_MSG -> {
                val mUserBlockEventHolder = mHolder as UserBlockEventHolder
                mUserBlockEventHolder.bind(user.isBlocked)
            }
            else -> {
    ...
    private fun getHolderByType(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        when (viewType) {
            ...
            MessageType.DISAPPEAR_MAG -> return DisappearHolder(context,LayoutInflater.from(parent.context).inflate(R.layout.disappear_msg, parent, false))
            ...
            MessageType.USER_BLOCKED_MSG -> return UserBlockEventHolder(context, LayoutInflater.from(parent.context).inflate(R.layout.row_user_blocked, parent, false),mOnClickListener)
        }
